...
//                    }
        ...
        for (LitemallApplicant applicant : result) {
            if (applicant.getSubmitStatus() >= 9 && CheckBizTypeUtil.hasPermission(roleIds, bizTypes, applicant.getApplicantType())) {
    ...
    public Object createAudit(@RequestBody LitemallApplicantBank applicantBank) {
        ...
        if (applicant.getSubmitStatus() == 9 && applicantBank.getStatus() == 2) {
            //有一家银行受理，改变申请人状态
            applicant.setSubmitStatus(11);
        } else if (applicant.getSubmitStatus() == 9 && applicantBank.getStatus() == 1){
            applicant.setSubmitStatus(10);
        }
