...
<script>
import { readDocument, updateDocument } from '@/api/document'
  ...
  data() {
    return {
      ...
      rules: {
        ...
        docType: [
          { required: true, message: '专题子标题不能为空', trigger: 'change' }
        ],
        detail: [
  ...
  methods: {
    ...
    handleConfirm() {
      this.$refs['document'].validate(valid => {
        if (valid) {
          updateDocument(this.document).then(response => {
            this.$router.push({ path: '/sys/document' })
          })
            .catch(response => {
              this.$notify.error({
