  ...
  <div class="app-container">
    <el-card class="box-card">
      ...
      <el-form ref="goods" :model="goods" label-width="180px">
        ...
        <el-row v-if="goods.applicantType == 'company' || goods.applicantAmount >= 10">
          ...
          <el-col :span="6">
            <el-form-item label="带动人员工资流水" prop="employmentPromotionUrl">
    ...
    <el-card v-if="isBankHidden" class="box-card">
      ...
      >
        <el-row>
          <el-col :span="8">
            <el-form-item label="受理银行" prop="bName">
              <el-input v-model="bName" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="支行" prop="bSubBranch">
              <el-input v-model="bSubBranch" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="贷款金融" prop="bLoanFinance">
              <el-input v-model="bankForm.bLoanFinance" />
            </el-form-item>
          </el-col>
        </el-row>
        <!-- <el-row>
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="贷款发放日" prop="bLoanStartDate">
              <el-date-picker
                v-model="bankForm.bLoanStartDate"
                type="date"
                placeholder="选择日期"
                value-format="yyyy-MM-dd HH:mm:ss"
                :picker-options="pickerOptions"
          ...
          <el-col :span="8">
            <el-form-item label="贷款到期日" prop="bLoanEndDate">
              <el-date-picker
                v-model="bankForm.bLoanEndDate"
                type="date"
                placeholder="选择日期"
                value-format="yyyy-MM-dd HH:mm:ss"
                :picker-options="pickerOptions"
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item v-if="bankForm.bLoanVoucherUrl" label="补充资料">
              <el-image v-for="picUrl in bankForm.bLoanVoucherUrl" :key="picUrl" style="width:50px;height:50px; margin: 0px 10px" :src="picUrl" :preview-src-list="[picUrl]" />
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-card v-if="auditList.length > 0">
      <h3>审核记录</h3>
      <div class="block">
...
<script>
import { readApplicant, updateApplicant, readBank } from '@/api/applicantReview'
...
export default {
  ...
  data() {
    return {
      ...
      rensheForm: { hsRigsterDate: undefined, hsExtraPicUrl: [] },
      assureForm: { scLetterIntentUrl: '', value: '', picUrl: '', scExtraPicUrl: [] },
      bankForm: { bLoanVoucherUrlList: [] },
      pickerOptions: {
        disabledDate(time) {
    ...
    init: function() {
      ...
      readApplicant({ id: goodsId }).then(response => {
        ...
        if (parseInt(goAction) === 1) {
          ...
          if (response.data.data.bBankId) {
            readBank({ bankId: response.data.data.bBankId }).then(bankRes => {
              this.bName = bankRes.data.data.name
              this.bSubBranch = bankRes.data.data.subBranch
            })
          }
          this.bankForm.status = parseInt(goAction)
